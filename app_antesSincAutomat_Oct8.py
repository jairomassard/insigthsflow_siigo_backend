...
def create_app():
   ...
   # --- ENDPOINT: Catálogo de centros de costo para reporte consolidado ---
    ...
    def verificar_permisos_global():
        ...
        if request.method == "OPTIONS":  # 👈 ignora preflight CORS
            return
    ...
    def siigo_sync_all():
        # No usamos @jwt_required porque podría llamarse desde el script sin token
        ...
        # Lista de los endpoints en orden con sus parámetros si aplica
        sequence = [
            ("/siigo/sync-catalogos", {}),
            ("/siigo/sync-customers", {}),
            ("/siigo/sync-proveedores", {}),
            ("/siigo/sync-productos", {}),
            ("/siigo/sync-facturas", {}),  # ligero
            ("/siigo/sync-facturas", {"deep": 1, "batch": 100, "only_missing": 1}),  # detallado
        ...
        logrec = SiigoSyncLog(
            idcliente = idcliente,
            fecha_programada = config.ultimo_ejecutado or func.now(),
