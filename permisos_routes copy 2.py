...
def register_permisos_routes(app):
    ...
    def obtener_permisos_por_perfil(idperfil):
        ...
        # 🔹 Traer todos los permisos del cliente, con info de si están asignados o no
        perfil_permisos = (
            db.session.query(
                Permiso.idpermiso,
                Permiso.nombre,
            ...
            .filter(Permiso.idcliente == idcliente)
            .order_by(Permiso.codigo.asc())
            .all()
        )

        result = []
        for pid, nombre, codigo, descripcion, permitido in perfil_permisos:
            result.append({
                "idpermiso": pid,
                "nombre": nombre,
                "codigo": codigo,
        ...
        print(f"[DEBUG] Cliente {idcliente}: retornando {len(result)} permisos para perfil {idperfil}")
