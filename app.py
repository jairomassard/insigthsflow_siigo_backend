...
def recalcular_ajustes_compras_cliente(idcliente):
    from sqlalchemy import text

    # 1. Resetea compras del cliente que NO tienen ajustes asociados.
    db.session.execute(text("""
        UPDATE siigo_compras c
        SET
            total_ajustes_debito = 0,
            total_ajustado = c.total,
            estado_ajuste = 'sin_ajuste',
            ajustes_count = 0,
            ajustes_updated_at = NOW()
        WHERE c.idcliente = :idcliente
          AND NOT EXISTS (
              SELECT 1
              FROM siigo_compras_ajustes a
              WHERE a.idcliente = c.idcliente
                AND a.documento_origen = c.idcompra
          )
    """), {"idcliente": idcliente})

    # 2. Aplica los ajustes existentes a las compras relacionadas.
    db.session.execute(text("""
        WITH ajustes AS (
            SELECT
                idcliente,
                documento_origen,
                SUM(COALESCE(total_ajuste, 0)) AS total_ajustes,
                COUNT(*) AS ajustes_count
...
def create_app():
    ...
    # =========================
    # Temporarl para la nueva prueba de autenticiocn con ]Siigo desde Junio 11 2026
    # =========================

    @app.route("/siigo/debug-auth-real", methods=["GET"])
    def siigo_debug_auth_real():
        debug_key = request.headers.get("X-Debug-Key", "")
        expected_key = os.getenv("DEBUG_SIIGO_KEY", "")

        if not expected_key or debug_key != expected_key:
            return jsonify({"error": "No autorizado"}), 403

        idcliente = request.args.get("idcliente", type=int)
        if not idcliente:
            return jsonify({"error": "Falta idcliente. Ej: /siigo/debug-auth-real?idcliente=13"}), 400

        cred = SiigoCredencial.query.filter_by(idcliente=idcliente).first()
        if not cred:
            return jsonify({"error": "Credenciales Siigo no configuradas para este cliente"}), 400

        access_key = dec(cred.client_secret)
        if not access_key:
            return jsonify({"error": "No se pudo desencriptar client_secret / access_key"}), 400
        ...
        resultado = {
            ...
            "auth_ok": r_auth.status_code in (200, 201),
            "auth_response_preview": r_auth.text[:500] if r_auth.status_code not in (200, 201) else "Token recibido correctamente",
        }

        if r_auth.status_code not in (200, 201):
            return jsonify(resultado), 200

        token_data = r_auth.json() or {}
        token = token_data.get("access_token")

        resultado["token_recibido"] = bool(token)
        resultado["token_type"] = token_data.get("token_type")
        resultado["expires_in"] = token_data.get("expires_in")

        if not token:
            resultado["error"] = "Siigo autenticó pero no devolvió access_token"
            return jsonify(resultado), 200
        ...
        for nombre_endpoint, path in {
            "users": "/v1/users",
            "customers_1": "/v1/customers?page=1&page_size=1",
            "products_1": "/v1/products?page=1&page_size=1",
        }.items():
            ...
            pruebas[nombre_endpoint] = {
                "bearer_status": r_bearer.status_code,
                "bearer_response": r_bearer.text[:700],
                "raw_status": r_raw.status_code,
                "raw_response": r_raw.text[:700],
            }
    ...
    def get_clientes_insights():
        ...
        try:
            ...
            # Aquí viven saldos, vencimientos, pagos y estados de cartera.
            # ============================================================
            ...
            # ============================================================
            # 10) Movimientos recientes comerciales por cliente
            # Fuente: ventas_movimientos_enriquecidos
            # Incluye FACTURA + NOTA_CREDITO
            # ============================================================

            params_movimientos_recientes = dict(params_mov)
            params_movimientos_recientes["limit_facturas"] = limit_facturas
            params_movimientos_recientes["mostrar_todas_facturas"] = mostrar_todas_facturas
            ...
            sql_facturas = text(f"""
                WITH movimientos AS (
                    SELECT
                        ...
                        CASE
                            WHEN m.tipo_movimiento = 'FACTURA'
                            THEN COALESCE(m.pagos_total, 0)
                            ELSE 0
                        END AS pagado,

                        CASE
                            WHEN m.tipo_movimiento = 'FACTURA'
                            THEN COALESCE(m.saldo, 0)
                            ELSE 0
                        END AS pendiente,
                        ...
                        CASE
                            WHEN m.tipo_movimiento = 'NOTA_CREDITO' THEN 'no_aplica'
                            WHEN ROUND(COALESCE(m.saldo, 0)::numeric, 2) = 0 THEN 'pagado'
                            WHEN m.vencimiento IS NOT NULL
                                AND m.vencimiento < CURRENT_DATE
                                AND COALESCE(m.saldo, 0) > 0 THEN 'vencido'
                            WHEN m.vencimiento IS NOT NULL
                                AND m.vencimiento >= CURRENT_DATE
                                AND m.vencimiento <= CURRENT_DATE + INTERVAL '5 days'
                                AND COALESCE(m.saldo, 0) > 0 THEN 'alerta'
                            ELSE 'sano'
                        END AS estado_cartera,

                        CASE
                            WHEN m.tipo_movimiento = 'NOTA_CREDITO' THEN NULL
                            WHEN m.vencimiento IS NULL THEN NULL
                            ELSE (m.vencimiento - CURRENT_DATE)
                        END AS dias_vencimiento,

                        ROW_NUMBER() OVER (
                            PARTITION BY m.cliente_nombre
                ...
                ORDER BY cliente_nombre, fecha DESC, tipo_movimiento ASC, documento DESC
            """)

            facturas_rows = [
                dict(r) for r in db.session.execute(sql_facturas, params_movimientos_recientes).mappings().all()
            ]
            ...
            for r in facturas_rows:
                ...
                tipo_movimiento = r.get("tipo_movimiento") or "FACTURA"
                es_nota_credito = tipo_movimiento == "NOTA_CREDITO"

                subtotal_f = money_float(r.get("subtotal"))
                impuestos_f = money_float(r.get("impuestos"))
                total_real = money_float(r.get("total"))

                # Para mostrar la nota crédito positiva en el listado.
                # Pero ventas_netas conserva el valor real: FACTURA suma, NOTA_CREDITO resta.
                total_mostrar = abs(total_real) if es_nota_credito else total_real
                ventas_netas_f = total_real

                valor_pagado = money_float(r.get("valor_pagado"))
                valor_pendiente = money_float(r.get("valor_pendiente"))
                ...
                facturas_por_cliente[cliente_key].append({
                    ...
                    "documento_afectado": r.get("documento_afectado"),
                    "tipo_movimiento": tipo_movimiento,
                    "tipo_documento_label": "Nota crédito" if es_nota_credito else "Factura",

                    "fecha": fecha_str,
                    "vencimiento": None if es_nota_credito else vencimiento_str,

                    "ventas_netas": ventas_netas_f,
                    "subtotal": subtotal_f,
                    "impuestos": impuestos_f,
                    "total_facturado_siigo": total_mostrar,

                    "ventas_netas_str": money_fmt(ventas_netas_f),
                    "subtotal_str": money_fmt(subtotal_f),
                    "impuestos_str": money_fmt(impuestos_f),
                    "total_facturado_siigo_str": money_fmt(total_mostrar),

                    "total": total_mostrar,
                    "total_str": money_fmt(total_mostrar),

                    "pagado": valor_pagado,
                    "pendiente": valor_pendiente,
                    "valor_pagado": valor_pagado,
                    "valor_pendiente": valor_pendiente,
                    "pagado_str": money_fmt(valor_pagado),
                    "pendiente_str": money_fmt(valor_pendiente),
                    ...
                    "estado_cartera": "no_aplica" if es_nota_credito else (r.get("estado_cartera") or "sano"),
            ...
            return jsonify({
                ...
                "config": {
                    ...
                    "detalle_cliente": "FACTURA + NOTA_CREDITO",
                }
            })
    ...
    def reportes_productos():
        ...
        # Aceptamos ambos nombres porque algunas páginas usan centro_costos
        centro_costo = (
            request.args.get("centro_costo")
            or request.args.get("centro_costos")
        )
        ...
        if desde:
            filtros.append("mb.fecha >= :desde")
            params["desde"] = desde

        if hasta:
            filtros.append("mb.fecha <= :hasta")
            params["hasta"] = hasta
        ...
        columna_orden = (
            ...
            else "SUM(COALESCE(i.total_item, 0))"
        )
        ...
        sql_kpis = text(f"""
            SELECT
                ...
                COALESCE(SUM(i.cantidad), 0) AS unidades_facturadas,
                COUNT(DISTINCT mb.movimiento_id) AS facturas,
                CASE
                    WHEN COUNT(DISTINCT mb.movimiento_id) > 0
                    THEN COALESCE(SUM(i.total_item), 0) / COUNT(DISTINCT mb.movimiento_id)
                    ELSE 0
                END AS ticket_promedio
    ...
    def detalle_producto():
        ...
        sql_hist = text(f"""
            SELECT
                DATE_TRUNC('month', mb.fecha) AS mes,
                COALESCE(SUM(i.cantidad), 0) AS cantidad,
                COALESCE(SUM(i.total_item), 0) AS total,
                COALESCE(SUM(i.total_item), 0) AS ventas_facturadas,
                COUNT(DISTINCT mb.movimiento_id) AS facturas
            ...
            AND i.idcliente = mb.idcliente
            WHERE {where}
            GROUP BY DATE_TRUNC('month', mb.fecha)
            ORDER BY mes
        """)
    ...
    def get_analisis_variacion_v1():
        ...
        # Este endpoint era una copia inline byte-a-byte de
        # construir_pnl_auxiliares (nunca se refactorizo para reutilizarla) -
        # corregido 2026-07-10 para usar el dispatcher construir_pnl y asi
        # heredar automaticamente tanto la version Alegra (derivada de
        # facturas) como la preferencia por Libro Diario real cuando existe,
        # igual que /reportes/pnl_v1. Cero cambio de comportamiento para
        # Siigo (misma funcion que ya usaba, solo que compartida en vez de
        # duplicada).
        return jsonify(construir_pnl(idcliente, desde, hasta)), 200
...
# No tocar de qui para abajo 
    ...
    def verificar_permisos_global():
        ...
        # ======================================================
        # Importaciones / cargues manuales
        # ======================================================
        elif request.path.startswith("/importar"):
            path_norm = request.path.lower().replace("_", "-")

            # ------------------------------------------
            # Notas débito de compras / documento soporte
            # Se carga desde Integración Siigo porque Siigo API
            # no expone esta información de forma confiable.
            # ------------------------------------------
            if (
                "notas-debito-compras-excel" in path_norm
                or "notas-debito-compras" in path_norm
                or "nota-debito-compras" in path_norm
                or "ajustes-compras" in path_norm
            ):
                codigo = "ver_siigo"
            ...
            # ------------------------------------------
            # Documento Soporte desde Excel - contingencia
            # Como vive en Integración Siigo, lo protegemos con ver_siigo.
            # ------------------------------------------
            elif (
                "soporte-excel" in path_norm
                or "documentos-soporte" in path_norm
                or "documento-soporte" in path_norm
            ):
                codigo = "ver_siigo"
            ...
            # ------------------------------------------
                ...
                return jsonify({
                    "error": "Ruta de importación no mapeada en control de permisos.",
                    "ruta": request.path,
                    "motivo": "importacion_sin_mapeo",
                    "recomendacion": (
