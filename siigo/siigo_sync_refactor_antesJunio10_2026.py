...
def _d(v, default=DZERO):
    """
    Convierte a Decimal de forma segura.

    Permite default=None para casos donde queremos distinguir:
    - Valor no recibido desde Siigo
    - Valor recibido en cero
    """
    try:
        if v is None or v == "":
            return default
...
def _parse_seller(seller_val):
    ...
    if isinstance(seller_val, dict):
        name = _str(
            seller_val.get("name")
            or seller_val.get("full_name")
            or seller_val.get("display_name")
        )
...
def _fetch_invoice_detail_safe(
    base_url: str,
    token: str,
...
def _needs_enrichment(f: SiigoFactura) -> bool:
    """
    Criterio técnico de enriquecimiento.

    Importante:
    No usa saldo ni estado_pago porque una factura puede estar
    perfectamente detallada y seguir pendiente de pago.
    """
    return (
        f.subtotal is None
        or f.impuestos_total is None
        or f.descuentos_total is None
        or f.pagos_total is None
        or f.moneda is None
    )
...
def sync_facturas_desde_siigo(
    ...
    def sum_subtotal_from_items(items: List[Dict[str, Any]]) -> Decimal:
        return sum(
            (_d(it.get("price"), DZERO) * _d(it.get("quantity"), DONE))
            for it in items
        )
    ...
    if not deep:
        ...
        for it in invoices_list:
            try:
                ...
                date = siigo_date_to_utc(
                    it.get("date"),
                    cliente.timezone or "America/Bogota",
                )
                ...
                if isinstance(payments, list) and payments and isinstance(payments[0], dict):
                    vencimiento = siigo_date_to_utc(
                        payments[0].get("due_date"),
                        cliente.timezone or "America/Bogota",
                    )
                ...
                if name:
                    f = SiigoFactura.query.filter_by(
                        idcliente=idcliente,
                        idfactura=name,
                    ).first()
                ...
                if not f:
                    ...
                    if vencimiento and f.vencimiento != vencimiento:
                    ...
                    if customer_name and (f.cliente_nombre or "") != customer_name:
                        f.cliente_nombre = customer_name
                        changed = True
                    ...
                    if customer_ident and (f.customer_identificacion or "") != customer_ident:
                        f.customer_identificacion = customer_ident
                        changed = True

                    if seller_id_val and f.seller_id != seller_id_val:
                        f.seller_id = seller_id_val
                        changed = True
        ...
        return (
            f"Ligero: {nuevas} nuevas, {actualizadas} actualizadas, "
            f"total procesadas {len(invoices_list)}."
        )
    ...
    def _query_facturas_pendientes_detalle():
        """
        Pendiente de detalle debe significar que faltan campos técnicos
        de enriquecimiento, NO que la factura esté pendiente de pago.

        Una factura puede estar correctamente detallada y seguir pendiente
        de pago. Esa factura NO debe reprocesarse indefinidamente.
        """
        q_pend = SiigoFactura.query.filter_by(idcliente=idcliente)

        if since:
            try:
                since_dt = datetime.fromisoformat(since).date()
                q_pend = q_pend.filter(SiigoFactura.fecha >= since_dt)
            except Exception:
                pass

        q_pend = q_pend.filter(
            (SiigoFactura.subtotal.is_(None))
            | (SiigoFactura.impuestos_total.is_(None))
            | (SiigoFactura.descuentos_total.is_(None))
            | (SiigoFactura.pagos_total.is_(None))
            | (SiigoFactura.moneda.is_(None))
        )

        return q_pend

    def _query_facturas_scope_total():
        q_total = SiigoFactura.query.filter_by(idcliente=idcliente)

        if since:
            try:
                since_dt = datetime.fromisoformat(since).date()
                q_total = q_total.filter(SiigoFactura.fecha >= since_dt)
            except Exception:
                pass
    ...
    if not objetivos:
        ...
        detalladas_total = max(total_facturas_scope - pendientes_despues, 0)
        ...
        return (
            f"No hay facturas pendientes de enriquecer. "
            f"Total facturas consideradas: {total_facturas_scope}. "
            f"Detalladas: {detalladas_total}. "
            f"Pendientes: {pendientes_despues}. "
            f"Progreso: {progreso}%. "
            f"Finalizado: sí."
        )
    ...
    for f in objetivos:
        ...
        try:
            detalle = _fetch_invoice_detail_safe(
                cred.base_url,
                token,
                inv_id,
            )
        ...
        f.customer_identificacion = _str(
            customer_raw.get("identification") or f.customer_identificacion or ""
        )
        ...
        # IMPORTANTE:
        # Guardamos ceros reales. Antes, valores como pagos_total=0 o iva_total=0
        # podían quedar como None y la factura seguía apareciendo como pendiente
        # de enriquecer indefinidamente.
        f.subtotal = subtotal
        f.impuestos_total = iva_total
        f.descuentos_total = descuentos_total
        f.pagos_total = pagos_total
        f.total = total_det if total_det is not None else (f.total or DZERO)
        ...
        f.medio_pago = _str(
            payments[0].get("name")
            if (
                isinstance(payments, list)
                and payments
                and isinstance(payments[0], dict)
            )
            else (f.medio_pago or None)
        )
        ...
        f.metadata_created = (
            ...
            if created
            else f.metadata_created
        )

        f.metadata_updated = (
            datetime.fromisoformat(updated.replace("Z", "+00:00"))
            if updated
            else f.metadata_updated
        )
        ...
        if items_detalle:
            ...
            for it in items_detalle:
                ...
                if total_item is None:
                    total_item = _d(it.get("price"), DZERO) * _d(
                        it.get("quantity"),
                        DONE,
                    )
    ...
    if finalizado:
        return (
            f"Detallado finalizado. "
            f"En este lote se enriquecieron {enriquecidas} facturas. "
            f"Fallidas en este lote: {fallidas}. "
            f"Total facturas consideradas: {total_facturas_scope}. "
            f"Detalladas: {detalladas_total}. "
            f"Pendientes: 0. "
            f"Progreso: 100%. "
    ...
    return (
        f"Detallado parcial. "
        f"En este lote se enriquecieron {enriquecidas} facturas de máximo {batch_size}. "
        f"Fallidas en este lote: {fallidas}. "
        f"Pendientes antes del lote: {pendientes_antes}. "
        f"Pendientes después del lote: {pendientes_despues}. "
        f"Total facturas consideradas: {total_facturas_scope}. "
        f"Detalladas: {detalladas_total}. "
        f"Progreso: {progreso}%. "
        f"Finalizado: no. "
        f"Ejecuta nuevamente 'Sincronizar detalle' para continuar."
    )


def contar_facturas_pendientes(idcliente: int, since: Optional[str] = None) -> int:
    """
    Cuenta facturas pendientes de detalle técnico.

    No usa saldo ni estado_pago porque una factura puede estar detallada
    correctamente y seguir pendiente de pago.
    """
    q = SiigoFactura.query.filter_by(idcliente=idcliente)
